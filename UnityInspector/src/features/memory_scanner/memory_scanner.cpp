...
namespace
{
	template <typename T> bool SafeRead(void* ptr, int offset, T& outValue)
	{
	...
	template <typename T> bool SafeWrite(void* ptr, int offset, const T& value)
	{
		if (!ptr || offset < 0) return false;
		__try
		{
...
void MemoryScanner::Render()
{
	...
	if (ImGui::Begin("Memory Scanner", &Config::settings.memoryScanner.showWindow))
	{
		...
		const char* compNames[] = {"Exact Match", "Increased Value", "Decreased Value", "Changed Value",
		                           "Unchanged Value"};
		...
		ImGui::SetNextItemWidth(-1.0f);
		ImGui::InputTextWithHint("##Filter", "Filter results by class, field, namespace, or value...",
		                         resultFilterBuffer, sizeof(resultFilterBuffer), ImGuiInputTextFlags_EscapeClearsAll);
		ImGui::Spacing();
		...
		if (selectedResultIndex >= 0 && selectedResultIndex < static_cast<int>(currentResults.size()))
		{
			...
			ImGui::Text("Selected: %s::%s (%s)", result.className.c_str(), result.fieldName.c_str(),
			            GetActualFieldTypeName(result.actualType));
			if (result.isStatic)
				ImGui::TextDisabled("Location: Static Field");
			else
				ImGui::TextDisabled("Location: Object %s (%p), Offset: 0x%X", result.objectName.c_str(), result.object,
				                    result.offset);
			...
			{
				valueChanged = ImGui::InputFloat("New Value", &editValue.f32, 0.0f, 0.0f, "%.3f",
				                                 ImGuiInputTextFlags_EnterReturnsTrue);
			}
			else if (result.actualType == ActualFieldType::Double)
			{
				valueChanged = ImGui::InputDouble("New Value", &editValue.f64, 0.0, 0.0, "%.6f",
				                                  ImGuiInputTextFlags_EnterReturnsTrue);
			}
			else if (result.actualType == ActualFieldType::ULong)
			{
				valueChanged = ImGui::InputScalar("New Value", ImGuiDataType_U64, &editValue.u64, nullptr, nullptr,
				                                  nullptr, ImGuiInputTextFlags_EnterReturnsTrue);
			}
			else
			{
				valueChanged = ImGui::InputScalar("New Value", ImGuiDataType_S64, &editValue.i64, nullptr, nullptr,
				                                  nullptr, ImGuiInputTextFlags_EnterReturnsTrue);
			}
...
void MemoryScanner::SyncValueToBuffer()
{
	switch (selectedType)
	{
		...
		case ScanValueType::Long:
			...
			break;
		case ScanValueType::Float: snprintf(m_valueBuffer, sizeof(m_valueBuffer), "%.3f", typedValue.f32); break;
		case ScanValueType::Double: snprintf(m_valueBuffer, sizeof(m_valueBuffer), "%.6f", typedValue.f64); break;
		case ScanValueType::Bool:
...
void MemoryScanner::PerformFirstScan()
{
	...
	if (comparison == ScanComparison::Exact)
	{
		...
		{
			case ScanValueType::Int:
			case ScanValueType::Long: valid = true; break;
			case ScanValueType::Float:
			{
				float f;
	...
	if (debugLogging) printf("[MemoryScanner] First scan: %s = %s\n", GetValueTypeName(selectedType), m_valueBuffer);

	statusText = "Scanning static fields...";
	ScanStaticFields(tempResults);
	if (debugLogging) printf("[MemoryScanner] Static matches: %zu\n", tempResults.size());
	...
	if (!stopRequested)
	{
		...
		if (debugLogging) printf("[MemoryScanner] Total matches: %zu\n", currentResults.size());
...
void MemoryScanner::PerformNextScan()
{
	...
	for (auto& field : localResults)
	{
		...
		if (!ReadFieldValue(field, currentValue)) continue;

		bool match = false;

		switch (comparison)
		{
			case ScanComparison::Exact: match = CompareValueWithTarget(currentValue, field.actualType); break;
			case ScanComparison::Increased: match = CompareValueWithPrevious(currentValue, field) > 0; break;
			case ScanComparison::Decreased: match = CompareValueWithPrevious(currentValue, field) < 0; break;
			case ScanComparison::Changed: match = CompareValueWithPrevious(currentValue, field) != 0; break;
			case ScanComparison::Unchanged: match = CompareValueWithPrevious(currentValue, field) == 0; break;
		}
...
void MemoryScanner::ScanStaticFields(std::vector<ScanField>& out) const
{
	...
	for (const auto& assembly : UR::assembly)
	{
		...
		{
			if (stopRequested) return;
			if (!klass || !klass->address) continue;

			if (klass->m_name.find('`') != std::string::npos || klass->m_name.find('<') != std::string::npos ||
			...
			for (const auto& field : klass->fields)
			{
				...
				if (!TypeNameMatchesSearchType(typeName)) continue;

				ActualFieldType actualType = DetermineActualFieldType(typeName);
				char rawBytes[8] = {};
				if (!ReadStaticFieldValue(field->address, rawBytes)) continue;
				...
				switch (actualType)
				{
					case ActualFieldType::Byte:
						*reinterpret_cast<int64_t*>(value) =
						    static_cast<int64_t>(*reinterpret_cast<uint8_t*>(rawBytes));
						break;
					...
					case ActualFieldType::Short:
						*reinterpret_cast<int64_t*>(value) =
						    static_cast<int64_t>(*reinterpret_cast<int16_t*>(rawBytes));
						break;
					case ActualFieldType::UShort:
						*reinterpret_cast<int64_t*>(value) =
						    static_cast<int64_t>(*reinterpret_cast<uint16_t*>(rawBytes));
						break;
					case ActualFieldType::Int:
						*reinterpret_cast<int64_t*>(value) =
						    static_cast<int64_t>(*reinterpret_cast<int32_t*>(rawBytes));
						break;
					case ActualFieldType::UInt:
						*reinterpret_cast<int64_t*>(value) =
						    static_cast<int64_t>(*reinterpret_cast<uint32_t*>(rawBytes));
						break;
				...
				if (!CompareValueWithTarget(value, actualType)) continue;
...
void MemoryScanner::ScanUnityObjectFields(std::vector<ScanField>& out)
{
	...
	{
		if (debugLogging) printf("[MemoryScanner] No Unity objects found to scan\n");
		return;
	}

	if (debugLogging) printf("[MemoryScanner] Scanning %zu Unity objects...\n", objectsGathered.size());
	...
	for (void* obj : objectsGathered)
	{
		...
		if (UT::String* nameStr = nullptr; Helper::SafeGetName(static_cast<UnityObject*>(obj), nameStr) && nameStr)
		{
		...
		{
			statusText =
			    "Scanning objects... " + std::to_string(processed) + "/" + std::to_string(objectsGathered.size());
		}
	}

	objectsGathered.clear();

	if (debugLogging) printf("[MemoryScanner] Scanned %d objects, found %zu matches\n", processed, out.size());
}
	...
	while (currentClass)
	{
		...
		if (!includeSystemNamespaces)
		{
			...
			if (ns && (std::string_view(ns).starts_with("System.") ||
			           std::string_view(ns).starts_with("UnityEngine.") || std::string_view(ns).starts_with("Unity.")))
			{
				currentClass =
		...
		while ((field = UR::Invoke<void*, void*, void*>(mono ? "mono_class_get_fields" : "il2cpp_class_get_fields",
		                                                currentClass, &iter)))
		{
			if (stopRequested) return;
			if (out.size() >= MAX_RESULTS) break;
			...
			const char* fieldName =
			    UR::Invoke<const char*, void*>(mono ? "mono_field_get_name" : "il2cpp_field_get_name", field);
			int offset = UR::Invoke<int, void*>(mono ? "mono_field_get_offset" : "il2cpp_field_get_offset", field);
			int flags = UR::Invoke<int, void*>(mono ? "mono_field_get_flags" : "il2cpp_field_get_flags", field);

			if ((flags & 0x10) != 0) continue;

			void* fieldType = UR::Invoke<void*, void*>(mono ? "mono_field_get_type" : "il2cpp_field_get_type", field);
			if (!fieldType) continue;

			const char* typeName =
			    UR::Invoke<const char*, void*>(mono ? "mono_type_get_name" : "il2cpp_type_get_name", fieldType);
			...
			if (std::string typeNameStr = typeName ? typeName : "unknown"; TypeNameMatchesSearchType(typeNameStr))
			{
				...
				if (ReadInstanceFieldValue(obj, offset, actualType, value))
				{
					if (CompareValueWithTarget(value, actualType))
					{
						...
						if (scanField.className.empty()) scanField.className = "Unknown";
						const char* ns = UR::Invoke<const char*, void*>(
						    mono ? "mono_class_get_namespace" : "il2cpp_class_get_namespace", currentClass);
			...
			else if (depth < MAX_SCAN_DEPTH)
			{
				...
				bool isDelegate =
				    typeNameStr.starts_with("System.") &&
				    (typeNameStr.find("Action") != std::string::npos || typeNameStr.find("Func") != std::string::npos ||
...
bool MemoryScanner::TypeNameMatchesSearchType(const std::string& typeName) const
{
	...
	{
		case ScanValueType::Int:
			return typeName == "System.Int32" || typeName == "int" || typeName == "System.UInt32" ||
			       typeName == "uint" || typeName == "System.Int16" || typeName == "short" ||
			       typeName == "System.UInt16" || typeName == "ushort" || typeName == "System.Byte" ||
			       typeName == "byte" || typeName == "System.SByte" || typeName == "sbyte";
		case ScanValueType::Long:
			return typeName == "System.Int64" || typeName == "long" || typeName == "System.UInt64" ||
			       typeName == "ulong";
		case ScanValueType::Float: return typeName == "System.Single" || typeName == "float";
		case ScanValueType::Double: return typeName == "System.Double" || typeName == "double";
		case ScanValueType::Bool: return typeName == "System.Boolean" || typeName == "bool";
	}
	return false;
}
...
bool MemoryScanner::ReadFieldValue(const ScanField& field, void* outValue) const
{
	...
	{
		char rawBytes[8] = {};
		if (!ReadStaticFieldValue(field.fieldHandle, rawBytes)) return false;
		...
		switch (field.actualType)
		{
			...
			case ActualFieldType::Double:
				*static_cast<double*>(outValue) = *reinterpret_cast<double*>(rawBytes);
				return true;
			case ActualFieldType::Bool: *static_cast<bool*>(outValue) = *reinterpret_cast<bool*>(rawBytes); return true;
		}
		return false;
	}
...
{
	if (!fieldHandle || !outValue) return false;
...
{
	if (!obj || offset < 0 || !outValue) return false;
	...
	switch (type)
	{
		...
		{
			uint8_t v;
			if (!SafeRead(obj, offset, v)) return false;
		...
		{
			bool v;
			if (!SafeReadBool(obj, offset, v)) return false;
...
{
	if (comparison != ScanComparison::Exact) return true;
	...
	switch (selectedType)
	{
		...
		{
			int64_t target;
			if (!GetTargetValueAsInt64(target)) return false;
			if (actualType == ActualFieldType::Byte || actualType == ActualFieldType::SByte ||
			    actualType == ActualFieldType::Short || actualType == ActualFieldType::UShort ||
			    actualType == ActualFieldType::Int || actualType == ActualFieldType::UInt ||
		...
		{
			float target;
			if (!GetTargetValueAsFloat(target)) return false;
			if (actualType == ActualFieldType::Float) return *static_cast<const float*>(value) == target;
			return false;
		}
		case ScanValueType::Double:
		{
			double target;
			if (!GetTargetValueAsDouble(target)) return false;
			if (actualType == ActualFieldType::Double) return *static_cast<const double*>(value) == target;
			return false;
		}
		case ScanValueType::Bool:
		{
			bool target;
			if (!GetTargetValueAsBool(target)) return false;
			if (actualType == ActualFieldType::Bool) return *static_cast<const bool*>(value) == target;
			return false;
		}
	}
	return false;
...
{
	if (!result.object && !result.isStatic) return;
...
std::vector<void*> MemoryScanner::GatherUnityObjects() const
{
	...
	if (!coreAssembly) return allObjects;

	auto objectClass = coreAssembly->Get("Object", "UnityEngine");
	if (!objectClass) return allObjects;
	...
	if (allObjects.size() > MAX_OBJECTS_TO_SCAN) allObjects.resize(MAX_OBJECTS_TO_SCAN);
...
std::string MemoryScanner::FormatValue(const ScanField::ValUnion& val, ActualFieldType type)
{
	...
	switch (type)
	{
		...
		case ActualFieldType::Int:
		case ActualFieldType::UInt:
		case ActualFieldType::Long: return std::to_string(val.i64);
		case ActualFieldType::ULong: return std::to_string(val.u64);
		case ActualFieldType::Float: snprintf(buf, sizeof(buf), "%.3f", val.f32); return buf;
		case ActualFieldType::Double: snprintf(buf, sizeof(buf), "%.6f", val.f64); return buf;
		case ActualFieldType::Bool: return val.b ? "true" : "false";
	}
	return "?";
}
...
bool MemoryScanner::WriteFieldValue(const ScanField& field, const void* valueBuffer) const
{
	if (field.isStatic)
	{
		...
		try
		{
			...
			{
				case ActualFieldType::Byte:
					*reinterpret_cast<uint8_t*>(rawBytes) =
					    static_cast<uint8_t>(*static_cast<const int64_t*>(valueBuffer));
					break;
				case ActualFieldType::SByte:
					*reinterpret_cast<int8_t*>(rawBytes) =
					    static_cast<int8_t>(*static_cast<const int64_t*>(valueBuffer));
					break;
				case ActualFieldType::Short:
					*reinterpret_cast<int16_t*>(rawBytes) =
					    static_cast<int16_t>(*static_cast<const int64_t*>(valueBuffer));
					break;
				case ActualFieldType::UShort:
					*reinterpret_cast<uint16_t*>(rawBytes) =
					    static_cast<uint16_t>(*static_cast<const int64_t*>(valueBuffer));
					break;
				case ActualFieldType::Int:
					*reinterpret_cast<int32_t*>(rawBytes) =
					    static_cast<int32_t>(*static_cast<const int64_t*>(valueBuffer));
					break;
				case ActualFieldType::UInt:
					*reinterpret_cast<uint32_t*>(rawBytes) =
					    static_cast<uint32_t>(*static_cast<const int64_t*>(valueBuffer));
					break;
				case ActualFieldType::Long:
			...
			{
				void* vTable = UR::Invoke<void*, void*, void*>(
				    "mono_class_vtable", UR::pDomain,
				    UR::Invoke<void*, void*>("mono_field_get_parent", field.fieldHandle));
				UR::Invoke<void, void*, void*, const void*>("mono_field_static_set_value", vTable, field.fieldHandle,
				                                            rawBytes);
			}
			else
			{
	...
	{
		case ActualFieldType::Byte:
			return SafeWrite(field.object, field.offset,
			                 static_cast<uint8_t>(*static_cast<const int64_t*>(valueBuffer)));
		case ActualFieldType::SByte:
			return SafeWrite(field.object, field.offset,
			                 static_cast<int8_t>(*static_cast<const int64_t*>(valueBuffer)));
